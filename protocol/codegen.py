...
C_TYPE = {
    ...
    "u32": "uint32_t", "i32": "int32_t", "u64": "uint64_t", "char": "char",
}
PY_FMT = {
    "u8": "B", "i8": "b", "u16": "H", "i16": "h",
    "u32": "I", "i32": "i", "u64": "Q", "char": "s",
}
...
def validate(spec: dict[str, Any]) -> list[str]:
    ...
    # --- buses -------------------------------------------------------------
    pin_fields = {
        f["name"] for f in spec["messages"]["BUS_CREATE"]["request"]
        if f["name"].endswith("_pin")
    }
...
def emit_c(spec: dict[str, Any]) -> str:
    ...
    o += ["/* ---- protocol ---- */",
          f"#define EMUWIRE_PROTOCOL_VERSION {p['version']}u", ""]
    o += ["/* ---- frame ---- */",
    ...
    o.append("/* ---- enums ---- */")
    for name, en in spec["enums"].items():
        note = f"  /* synthesised from {en['synthesised_from']}/ */" if "synthesised_from" in en else ""
    ...
    o += ["", "/* Bit 7 marks an asynchronous event: no lookup needed to route a frame. */",
    ...
    def emit_struct(struct_name: str, fields: list[dict], comment: str = "") -> None:
        ...
        for f in fields:
            if "count_field" in f:
                tail = f
                continue
            note = f"  /* {f['enum']} */" if "enum" in f else (
                f"  /* {f['bitmask']} */" if "bitmask" in f else "")
            if f["type"] == "struct":
        ...
        if tail is not None:
            ctype = (f"{c_name(tail['struct'])}_t" if tail["type"] == "struct"
                     else C_TYPE[tail["type"]])
...
def emit_py(spec: dict[str, Any]) -> str:
    o: list[str] = [BANNER_PY, "", "from __future__ import annotations", "",
                    "import struct", "from dataclasses import dataclass, field",
                    "from enum import IntEnum", "from typing import Any, ClassVar, Dict, List, Optional, Tuple", ""]

    p, fr, crc = spec["protocol"], spec["frame"], spec["crc"]
    o += [f"PROTOCOL_VERSION = {p['version']}", "",
    ...
    o += [f"# {crc['name']}: poly={crc['polynomial']:#06x} init={crc['init']:#06x} "
    ...
    # --- framing -----------------------------------------------------------
    o += ["def encode_frame(msg_type: int, seq: int, payload: bytes = b'') -> bytes:",
    ...
    def emit_dc(cls: str, fields: list[dict], type_id: int | None = None) -> None:
        ...
        if tail is not None:
            if tail["type"] == "struct":
                o.append(f"    {tail['name']}: List[{camel(tail['struct'])}] = field(default_factory=list)")
            else:
        ...
        o.append(f"                f'{cls} needs at least {{cls.FIXED_SIZE}} bytes, got {{len(data)}}'")
        o.append("            )")
        if fixed:
        ...
        if tail is not None:
            if tail["type"] == "struct":
                ...
                o.append(f"        blob = bytes(data[cls.FIXED_SIZE:cls.FIXED_SIZE + {tail['count_field']}])")
...
def main() -> int:
    ...
    counts = (f"{len(spec['messages'])} messages, {len(spec['enums'])} enums, "
